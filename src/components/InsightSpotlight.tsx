...
const insightHighlights = [
  {
    label: "意思決定の拠り所",
    value: "48時間で整理",
    caption: "市場・現場・資金の歪みを一画面に重ね、判断の焦点をそろえる",
  },
  {
    label: "設計図づくり",
    value: "AIと診断士が共同",
    caption: "数字と声を同時に扱い、納得感の高い選択肢を並べる",
  },
  {
    label: "会議のスピード",
    value: "初回会議で方向決定",
    caption: "共有テンプレートで前提をそろえ、議論を結論づくりに集中",
  },
];
...
const blueprintSignals = [
  {
    label: "粗利のゆとり",
    value: "改善余地を即表示",
    caption: "再投資に回せる力を見せ、議論の準備時間を短縮",
  },
  {
    label: "資金繰りの見通し",
    value: "先の波を俯瞰",
    caption: "AIが需給差を週次で知らせ、交渉準備を先回り",
  },
  {
    label: "推進チーム",
    value: "部署を横断",
    caption: "現場ヒアリングで制約を洗い出し、判断を一つにまとめる",
  },
];
...
const blueprintAssurances = [
  {
    title: "根拠の一体化",
    metric: "つながり可視化",
    description: "粗利・資金・現場指標の相関を色と位置で示し、判断の裏付けを共有。",
  },
  {
    title: "論点の揃え方",
    metric: "議題同期",
  ...
  },
  {
    title: "実行の強さ",
    metric: "現場適合",
    description: "部門別の制約と投資余地を同じ画面で示し、即時の合意形成につなげる。",
  },
];
...
const rootCauseNarrative = [
  {
    badge: "Pattern 01",
    title: "数字に込めた意図が全社へ届かない",
    description:
      "原価や人件費の波が点で管理され、部署ごとに優先順位がずれてしまう。決裁の遅れが将来資金のゆがみを呼び、守りたい価値まで霞みます。",
    metric: "判断リードタイム +2週間",
  ...
  },
  {
    ...
    description:
      "現場確認→資料作成→再説明の往復が続き、未来の構想に割く時間が消えていく。外部との調整に追われ、攻めの議論が止まります。",
    metric: "再説明工数 1.7倍",
  ...
  },
  {
    ...
    description:
      "改善策は揃っていても、人・数字・現場の視点が交わらず、語る未来に一体感が宿らない。実行に踏み切る勇気が薄れ、変革の初速が鈍ります。",
    metric: "再投資判断が停滞 3カ月",
...
const hopeStatements = [
  ...
  {
    title: "設計フェーズ",
    description: "生成AIと診断士がボトルネックを多面的に整理し、未来を描く核となる仮説を厳選。",
    metric: "短期集中診断",
    kpi: "48hで歪みの見取り図",
  },
  {
          ...
          <div className="mx-auto w-full max-w-2xl space-y-12 lg:max-w-none">
            <ScrollReveal variant="fade-up" className="space-y-4">
              ...
              <div className="space-y-4 text-[1.05rem] leading-relaxed text-[#1e3359]/80 md:text-lg">
                <p>
                  原価や採用の変化を把握していても、数字と現場の体感が別々に積み上がると次の打ち手に踏み切れません。資料づくりで一日が終わり、未来を描く時間が削られる──その繰り返しが意思決定の質と速さを鈍らせます。
                </p>
                <p>
                  私たちは、生成AIが読み解く兆しと中小企業診断士のヒアリングを重ね、経営者が誇りを持って語れる判断軸を整えます。経営会議で示すべき筋道と実行の段取りを一本化し、次の一手を胸を張って選べる状態を共につくります。
                </p>
                <p>
                ...
                <p>
                  計画策定後も中小企業診断士が伴走し、現場と経営が同じ道筋で動けるようサイクルを整えます。攻めと守りの判断を前向きに共有できる状態を保ち、周囲からの信頼を引き寄せます。
                </p>
              </div>
            </ScrollReveal>
