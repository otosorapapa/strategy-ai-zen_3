...
const painPoints: PainPoint[] = [
  {
    ...
    description:
      "粗利や資金の変化と現場の声が別々に届き、会議は数字合わせで時間が過ぎがちです。準備に追われるほど好機は後ろへ流れてしまいます。",
    signal: "粗利率の鈍化と運転資金の圧迫が同時進行（例）",
    insight: "報告が点在 → 根拠をそろえるまで時間がかかり、先手を打つ意志が鈍る。",
    icon: AlertTriangle,
  },
  {
    badge: "提供する価値",
    ...
    description:
      "業界・競合・顧客の動きを生成AIが整理し、診断士が現場条件と重ねて一枚の経営設計図にまとめます。どの選択肢が攻め手になるかが一目でわかります。",
    signal: "AI提案が会議で採用され、打ち手の比較が即断で進む",
    insight: "仮説と検証指標を同時提示 → 判断の拠り所がそろい、挑戦へ踏み出しやすくなる。",
    icon: AlertTriangle,
  },
  {
    badge: "実現の進め方",
    ...
    description:
      "経営管理アプリとダッシュボードを事業者ごとに調整し、会議メモから交渉資料まで一連で準備します。どの場でも落ち着いて語れる整った状態を保てます。",
    signal: "準備時間を圧縮し、外部との打ち合わせで質問が先読みできる",
    insight: "判断ログとタスクを一本化 → 行動の遅れを抑え、未来を描く会話を主導できる。",
    icon: AlertTriangle,
  },
];
  ...
  return (
    <section id="why-section" className="fade-in-section bg-[#04112a] py-24 text-white">
      ...
      <SectionMicroCta
        variant="dark"
        eyebrow="Micro CTA"
        title="課題が浮上したら、48時間診断で判断の優先度と根拠を確認"
        description="生成AIと診断士が業界・競合・顧客・コストのデータを束ね、今すぐ押さえるべき論点と次の会議で提示できる骨子を整えます。貴社専用の管理アプリと行動メモを受け取り、社内外へ胸を張って打ち手を示せる状態をつくりましょう。"
