...
const howPillars = [
  {
    ...
    title: "生成AIが未来を読む",
    description:
      "財務データ、販売記録、業界動向、経済指標をまとめて分析。粗利と資金繰りの歪みを「何が・いつ・どこで」起きているかまで因果分解し、進むべき方向を素早く示します。",
  },
  {
    icon: Users,
    title: "専門家が現場で翻訳",
    description:
      "中小企業診断士が現場ヒアリングとワークショップを実施。人と組織の温度感を踏まえ、AIが示した打ち手を実行可能な週次タスクへ置き換えます。",
  },
  {
    icon: Map,
    title: "再生ロードマップを設計",
    description:
      "投資・撤退・強化すべき領域を週次で可視化。社外への説明ストーリーと現場タスクを連動させ、実行状況と資金への影響を同時に追跡します。",
  },
];
...
const impactHighlights = [
  {
    label: "経営判断の狙い",
    value: "会議から判断までが加速",
    detail: "会議から意思決定までの時間短縮が進み、次の一手に迷わなくなります",
    icon: TimerReset,
  ...
  },
  {
    label: "収益の伸ばしどころ",
    value: "粗利改善の余地を見える化",
    detail: "重点施策がもたらす粗利益の押し上げ効果を根拠付きで把握できます",
    icon: TrendingUp,
  ...
  },
  {
    label: "社外への伝わり方",
    value: "資料整備が高評価",
    detail: "資料と数字の一貫性が伝わり、社外の信頼も揺らぎません",
    icon: ShieldCheck,
...
const logicSequence = [
  {
    title: "課題が見える化",
    description:
      "AIが粗利とキャッシュの歪みを採点し、要因を階層構造で提示。どこを優先すべきかがすぐに浮かび上がります。",
    metricLabel: "寄与要因の把握",
    metricValue: "高精度で可視化",
    icon: Layers,
  },
  {
    title: "打ち手を設計",
    description:
      "診断士がAIの試算を検証し、優先度とリスクを意思決定ボードに整理。現場タスクと指標が一体で動きます。",
    metricLabel: "実行準備",
    metricValue: "短期間で整う",
    icon: GanttChart,
  },
  {
    title: "成果を定着",
    description:
      "週次の進捗と社外向けの説明資料が同じ画面で更新。判断の裏付けを映し出すレポートが自動で整います。",
    metricLabel: "連携満足度",
    metricValue: "高水準を継続",
    icon: BarChart3,
  },
...
const solutionQuickSummary = [
  "生成AIが外部環境と財務データを重ね、危機の兆しと打つ手の方向性を整理",
  "診断士が現場ヒアリングで実行条件を確認し、未来像へ進む再生シナリオを描く",
  "経営管理アプリと資料を連動させ、社内外へ胸を張って語れる説明基盤を整える",
];
...
const causewayStages = [
  {
    id: "01",
    title: "課題の焦点を秒で捕捉",
    highlight: "粗利・キャッシュ・人材指標を同じ層で照合し、異常値を優先度順に提示。",
    kpi: "異常検知 92% 精度",
    description:
      "財務・販売・稼働データを暗号化連携し、AIが因果関係の網を構築。判断を止める論点を短時間で抽出します。",
  },
  {
    id: "02",
    title: "打ち手の妥当性を合意形成",
    ...
    kpi: "施策合意 2.3週",
    description:
      "週次レビューで仮説を検証し、投資判断に必要な根拠資料と現場タスクを一式そろえます。",
  },
  {
    id: "03",
    title: "成果の証跡を蓄積",
    highlight: "金融機関・取引先・現場の報告書式を統一し、納得度の高い数字だけを残す。",
    kpi: "レポート作成 65%削減",
    description:
      "実行ログとキャッシュ波形を同じ画面で追跡し、判断の確信を深めるストーリーを自動更新します。",
  },
];
...
const keyProofPoints = [
  {
    icon: Target,
    title: "因果で語れる経営指標",
    detail: "粗利・キャッシュ・人員の遅れ要因を一枚の図で比較。経営陣にも社員にもすぐに筋道を示せます。",
  },
  {
    icon: LineChart,
    title: "最短2週で仮説検証を開始",
    detail: "経営会議の議題を定型化し、意思決定までの時間を平均40%削減した実績を保持。",
  },
  {
    icon: ArrowUpRight,
    title: "金融機関への説明がスムーズ",
    detail: "モニタリング資料と現場タスクが同期し、融資交渉や条件調整の対話が短時間で整います。",
  },
];
...
const workflowSteps = [
  {
    title: "STEP 0｜初期診断",
    detail:
      "短期間で粗利・キャッシュ・人材の行き詰まりを抽出し、生成AIが描いた歪みのヒートマップを提示します。",
  },
  {
    title: "STEP 1｜再生設計ボードづくり",
    detail:
      "AIが生成したシナリオを専門家が検証。意思決定ボードに優先仮説とリスクシナリオを整理し、週次会議で共有。",
  },
  {
...
const implementationTimeline = [
  ...
  {
    phase: "Week 1-2",
    title: "再生設計ボードとKPIを合意",
    detail: "診断士がAI提案を検証し、粗利・資金・現場の指標を一本化。役員会向けの意思決定ボードを整備。",
    deliverable: "意思決定ボード／週次レビュー設計図",
    icon: ClipboardCheck,
  },
  {
    phase: "Week 3-4",
