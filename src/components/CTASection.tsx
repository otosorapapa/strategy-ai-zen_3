...
const requestOptions = [
  {
    ...
    title: "無料相談で方向性を定義",
    description:
      "経営者とともに現状を棚卸しし、“再生の設計図”のドラフトと優先仮説メモを速やかにご共有します。次の場で堂々と語れる要点を整理します。",
    badge: "無料相談",
  },
  {
    value: "diagnostic",
...
const CTASection = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "送信が完了しました",
        description: "担当よりできるだけ早くご連絡いたします。",
      });
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          ...
          >
            ...
            <div className="rounded-[28px] border border-white/10 bg-white/5 p-6 text-sm leading-relaxed text-slate-200/85">
              ...
              初回の無料相談では、粗利・キャッシュ・人材のギャップと優先施策を整理し、行動順序と金融機関への伝え方まで含めた“再生の設計図”をドラフトとしてお渡しします。次のステークホルダー面談で堂々と話せる根拠をご用意します。
            </div>
          </ScrollReveal>
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-[#0b1f3f]/10 bg-[#f6f9ff] p-4 text-xs text-[#1e3359]/80">
                ...
                <p>
                  送信後は、代表または専門チームより順次ご連絡します。お急ぎの方はお電話（092-231-2920）でも承ります。
                </p>
              </div>
            </form>
          </ScrollReveal>
